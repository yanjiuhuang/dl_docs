...
##### 其他输出单元类型
上文中描述的线性、sigmoid和softmax输出单元是最为常见的输出单元。实际上神经网络可以通用地使用任何类型的输出层。最大似然率的原则指导了针对任何类型的输出层如何设计一个好的损失函数。  

如果我们定义了一个条件分布$p(y | x; \theta)$，最大似然的原理告诉我们需要使用类似于$- \log p(y| x; \theta)$来作为损失函数。我们可以认为神经网络表示了一个函数$f(x;\theta)$。如果函数的输出不是对值$y$的直接预测，$f(x;\theta)=\omega$而是$y$所在分布的参数。我们的损失函数就可以是如下形式: $- \log p(y; \omega(x))$。  
